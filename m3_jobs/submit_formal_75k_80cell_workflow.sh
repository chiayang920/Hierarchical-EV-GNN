...
case "${ACTION}" in
  submit-smoke)
    ...
    sbatch --parsable \
      ...
      --mem="${SMOKE_MEM}" \
      --time="${SMOKE_TIME}" \
      --export="ALL,EV_GNN_FORMAL_75K_PYTHON=${EV_GNN_FORMAL_75K_PYTHON},EV_GNN_FORMAL_75K_RESOURCE_PROFILE=${RESOURCE_PROFILE},EV_GNN_FORMAL_75K_SOURCE_IDENTITY=${SOURCE_IDENTITY},EV_GNN_FORMAL_75K_SOURCE_BUNDLE_IDENTITY=${SOURCE_BUNDLE_IDENTITY}" \
      "${SMOKE_SCRIPT}"
    ;;
  smoke-gate)
    require_source_identity
  ...
  submit-eval)
    ...
    sbatch --parsable \
      ...
      --mem="${EVAL_MEM}" \
      --time="${EVAL_TIME}" \
      --export="ALL,EV_GNN_FORMAL_75K_PYTHON=${EV_GNN_FORMAL_75K_PYTHON},EV_GNN_FORMAL_75K_ARRAY_JOB_ID=${ARRAY_JOB_ID},EV_GNN_FORMAL_75K_RESOURCE_PROFILE=${RESOURCE_PROFILE},EV_GNN_FORMAL_75K_STAGE_ROOT=${STAGE_ROOT}" \
      "${EVAL_SCRIPT}"
    ;;
  submit-diagnostics)
    formal75k_require_numeric_id "training array job ID" "${ARRAY_JOB_ID}"
