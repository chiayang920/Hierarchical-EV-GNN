...
REQUIRED_HEAD_SHA="c72d7f5da6738da6561904b0840c2faf540b8262"
RECORDED_HEAD_SHA="${EV_GNN_DIAGNOSTIC_SMOKE_SOURCE_EXPECTED_HEAD_SHA:-${REQUIRED_HEAD_SHA}}"
BRANCH_REQUIRED="main"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd -P)"
CURRENT_DIR="$(pwd -P)"
TOP_LEVEL="EV-GNN-infrastructure-diagnostic-smoke-${RECORDED_HEAD_SHA}"
OUTPUT_ROOT="${EV_GNN_DIAGNOSTIC_SMOKE_SOURCE_OUTPUT_ROOT:-${REPO_ROOT}/source_bundles}"
...
is_prohibited_path() {
  ...
  case "${path}" in
    ...
    ._*|*/._*) return 0 ;;
    *.tar|*.tar.gz|*.tgz|*.zip|*.csv|*.pt|*.pth) return 0 ;;
...
print_transfer_commands() {
  ...
  echo "M3_EXTRACT_COMMAND=mkdir -p /scratch2/fr57/cche0357/EV-GNN_sources && tar -xzf /projects/fr57/cche0357/EV-GNN_sources/${archive_name} -C /scratch2/fr57/cche0357/EV-GNN_sources"
...
if [[ "${EV_GNN_DIAGNOSTIC_SMOKE_SOURCE_DRY_RUN:-0}" == "1" ]]; then
  ...
  echo "required_head_sha=${RECORDED_HEAD_SHA}"
...
[[ "${HEAD_SHA}" == "${RECORDED_HEAD_SHA}" ]] || die "HEAD ${HEAD_SHA} does not match required ${RECORDED_HEAD_SHA}"
...
if tar -tzf "${ARCHIVE_PATH}" | grep -E '(^|/)(\.git|saved_models|Downloads|task_packages|__pycache__|\.pytest_cache|__MACOSX)(/|$)|(^|/)\._|\.csv$|\.pt$|\.pth$|model\.best|model\.last' >/dev/null; then
