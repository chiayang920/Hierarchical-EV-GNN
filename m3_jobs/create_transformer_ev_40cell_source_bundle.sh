...
OUTPUT_ROOT="${EV_GNN_TRANSFORMER_EV_SOURCE_OUTPUT_ROOT:-${HOME}/Downloads/EVGNN_Transformer_EV_Evidence}"
EXPECTED_HEAD_SHA="${EV_GNN_TRANSFORMER_EV_SOURCE_EXPECTED_HEAD_SHA:-}"
RECORDED_HEAD_SHA=""
TOP_LEVEL=""
ARCHIVE_PATH=""
SHA_PATH=""
...
ALLOWLIST=(
  ...
  TD3/TD3_TransformerEVActionGNN.py
  ...
  m3_jobs/27_transformer_ev_40cell_smoke.slurm
  m3_jobs/28_transformer_ev_40cell_train.slurm
  m3_jobs/29_transformer_ev_40cell_eval30.slurm
  m3_jobs/30_transformer_ev_40cell_diagnostics.slurm
  m3_jobs/submit_transformer_ev_40cell_workflow.sh
  m3_jobs/create_transformer_ev_40cell_source_bundle.sh
  m3_jobs/lib_transformer_ev_runtime.sh
  scripts/transformer_ev_40cell_workflow.py
  scripts/transformer_ev_40cell_artifacts.py
  scripts/transformer_ev_40cell_diagnostic_validation.py
...
set_archive_paths() {
  TOP_LEVEL="EV-GNN-transformer-ev-40cell-${RECORDED_HEAD_SHA}"
...
if [[ "${EV_GNN_TRANSFORMER_EV_SOURCE_DRY_RUN:-0}" == "1" ]]; then
...
cd "${REPO_ROOT}"
[[ -n "${EXPECTED_HEAD_SHA}" ]] || die "EV_GNN_TRANSFORMER_EV_SOURCE_EXPECTED_HEAD_SHA is required"
[[ "$(git rev-parse HEAD)" == "${EXPECTED_HEAD_SHA}" ]] || die "HEAD does not match EV_GNN_TRANSFORMER_EV_SOURCE_EXPECTED_HEAD_SHA"
