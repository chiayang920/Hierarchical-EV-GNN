#!/usr/bin/env python3
"""Local contracts for the transformer-constraint 40-cell workflow."""
...
MODEL_NAMES = {
    ALGORITHM: "Full Hierarchical TD3 EV-GNN + Hard Transformer Feasibility Constraint",
}
...
KEY_MECHANISM_METRIC = "any_constraint_activation_step_fraction"
...
APPROVED_NEW_EVIDENCE_ROOT = (
    "/Users/jameschen/Desktop/MAIN_HOME/Monash/monash_course/2026S1/Research/"
    "Research_project/EVGNN_Research_Artefacts/00_current_authoritative_evidence/"
    "full_hierarchy_transformer_constraint_40cell_75k_20260825"
)
...
class TransformerConstraintCell:
    task_id: int
    scale: str
    algorithm: str
    seed: int
...
def validate_algorithm(algorithm: str) -> str:
    ...
    if algorithm != ALGORITHM:
        raise ValueError(f"unsupported transformer-constraint workflow algorithm: {algorithm!r}")
    return algorithm


def _make_cell(task_id: int, scale: str, seed: int, training_steps: int) -> TransformerConstraintCell:
    ...
    return TransformerConstraintCell(
        task_id=task_id,
        scale=scale,
        algorithm=ALGORITHM,
        seed=seed,
...
def formal_matrix() -> tuple[TransformerConstraintCell, ...]:
    cells: list[TransformerConstraintCell] = []
    task_id = 0
    for scale in SCALES:
        for seed in SEEDS:
...
def smoke_matrix() -> tuple[TransformerConstraintCell, ...]:
    return tuple(_make_cell(task_id, scale, 0, SMOKE_TRAINING_STEPS) for task_id, scale in enumerate(SCALES))


def resolve_formal_cell(task_id: int) -> TransformerConstraintCell:
    if type(task_id) is not int or not 0 <= task_id < EXPECTED_FORMAL_CELLS:
        raise ValueError(f"transformer-constraint formal task ID must be 0..39; got {task_id!r}")
    return formal_matrix()[task_id]


def resolve_smoke_cell(task_id: int) -> TransformerConstraintCell:
    if type(task_id) is not int or not 0 <= task_id < len(SCALES):
        raise ValueError(f"transformer-constraint smoke task ID must be 0..3; got {task_id!r}")
    return smoke_matrix()[task_id]


def format_cell_line(cell: TransformerConstraintCell) -> str:
    return (
...
def training_command(cell: TransformerConstraintCell, save_dir: str, run_name: str) -> list[str]:
...
def eval_seed_offset(cell: TransformerConstraintCell) -> int:
    return EVAL_SEED_BASE_BY_SCALE[cell.scale] + 999 * cell.seed


def canonical_episode_seed(
    cell: TransformerConstraintCell,
    episode_index: int,
) -> int:
    if type(episode_index) is not int or not 0 <= episode_index < FORMAL_MODEL_BEST_EVAL_EPISODES:
        raise ValueError(
            "formal eval30 episode index must be "
            f"0..{FORMAL_MODEL_BEST_EVAL_EPISODES - 1}; got {episode_index!r}"
        )
    return EVAL_SEED_BASE_BY_SCALE[cell.scale] + 1000 * cell.seed + episode_index


def canonical_episode_seeds(cell: TransformerConstraintCell) -> tuple[int, ...]:
    return tuple(
        canonical_episode_seed(cell, episode_index)
        for episode_index in range(FORMAL_MODEL_BEST_EVAL_EPISODES)
    )


def _require_model_best_checkpoint(checkpoint_prefix: str) -> None:
    if Path(str(checkpoint_prefix)).name != "model.best":
        raise ValueError("canonical eval30 and diagnostics require checkpoint prefix named model.best")
...
def eval30_command(
    ...
    _require_model_best_checkpoint(checkpoint_prefix)
    return [
        "python",
        "evaluate_td3_gnn.py",
        "--algorithm",
...
def projection_diagnostic_command(
    cell: TransformerConstraintCell,
    checkpoint_prefix: str,
    output_dir: str,
    run_name: str,
...
def runtime_gate_matrix() -> tuple[TransformerConstraintCell, ...]:
    return tuple(_make_cell(task_id, scale, 0, 4) for task_id, scale in enumerate(SCALES))


def resolve_runtime_gate_cell(task_id: int) -> TransformerConstraintCell:
    if type(task_id) is not int or not 0 <= task_id < len(SCALES):
        raise ValueError(f"transformer-constraint runtime-gate task ID must be 0..3; got {task_id!r}")
    return runtime_gate_matrix()[task_id]


def runtime_gate_command(cell: TransformerConstraintCell, save_dir: str, run_name: str) -> list[str]:
    return [
        "python",
        "train_td3_gnn.py",
        "--algorithm",
...
def _require_exact_cell_inventory(rows: Sequence[dict[str, object]], label: str) -> GateResult:
    ...
    if missing or duplicate or len(observed) != EXPECTED_FORMAL_CELLS:
        raise ValueError(
            f"{label} gate requires 40/40 unique transformer-constraint cells; "
...
def _validate_canonical_episode_seed(row: dict[str, object], label: str) -> None:
    scale, algorithm, seed = _row_cell_key(row)
    episode_index = _parse_int(row.get("episode_index"), "episode_index")
    cell_by_key = {
        (cell.scale, cell.algorithm, cell.seed): cell
        for cell in formal_matrix()
    }
    cell = cell_by_key[(scale, algorithm, seed)]
    observed = _parse_int(row.get("episode_seed"), f"{label} episode_seed")
    expected = canonical_episode_seed(cell, episode_index)
    if observed != expected:
        raise ValueError(
            f"{label} episode_seed mismatch for {scale} {algorithm} seed {seed} "
            f"episode {episode_index}: expected {expected}, observed {observed}"
        )


def validate_eval30_gate(rows: Sequence[dict[str, object]]) -> GateResult:
    gate = _validate_episode_inventory(rows, "transformer-constraint eval30", FORMAL_MODEL_BEST_EVAL_EPISODES)
    for row in rows:
        context = f"{row.get('scale')} {row.get('algorithm')} seed {row.get('seed')} episode {row.get('episode_index')}"
        _validate_canonical_episode_seed(row, "transformer-constraint eval30")
...
def validate_diagnostic_gate(rows: Sequence[dict[str, object]]) -> GateResult:
    gate = _validate_episode_inventory(rows, "transformer-constraint diagnostic", FORMAL_MODEL_BEST_EVAL_EPISODES)
    for row in rows:
        context = f"{row.get('scale')} {row.get('algorithm')} seed {row.get('seed')} episode {row.get('episode_index')}"
        _validate_canonical_episode_seed(row, "transformer-constraint diagnostic")
        if str(row.get("checkpoint_role", "")).strip() != "model.best":
            raise ValueError(f"{context}: model.best identity is required")
        _parse_float(row.get(KEY_MECHANISM_METRIC), f"{context} {KEY_MECHANISM_METRIC}")
        violation_count = _parse_int(
            row.get("transformer_feasibility_violation_count", 0),
            f"{context} transformer_feasibility_violation_count",
        )
        max_excess = _parse_float(
            row.get("transformer_feasibility_max_excess_kw", 0.0),
            f"{context} transformer_feasibility_max_excess_kw",
        )
        if violation_count != 0 or max_excess > 1e-9:
            raise ValueError(f"{context}: transformer feasibility invariant failed")
...
def dry_run_summary() -> list[str]:
    ...
    smoke = smoke_matrix()
    lines = [
        "PHASE=TRANSFORMER_CONSTRAINT_40CELL_WORKFLOW_DRY_RUN",
        f"FORMAL_MATRIX_CELL_COUNT={len(formal)}",
        f"FORMAL_MATRIX_UNIQUE_CELL_COUNT={len({(cell.scale, cell.algorithm, cell.seed) for cell in formal})}",
        "FORMAL_TASK_IDS=0..39",
        "FORMAL_ALGORITHM_SET=hierarchical_transformer_constraint",
        "FORMAL_EXCLUDED_ALGORITHMS=actiongnn,actiongnn_nonnegative,hierarchical,hierarchical_transformer_ev",
        "FORMAL_RESOURCE_RANGES=0-19:25cp_100cp,20-29:500cp,30-39:1000cp",
        f"HISTORICAL_FORMAL75K_ROOT={HISTORICAL_FORMAL75K_ROOT}",
        "HISTORICAL_FORMAL75K_OUTPUT_WRITE_ALLOWED=NO",
        f"APPROVED_NEW_EVIDENCE_ROOT={APPROVED_NEW_EVIDENCE_ROOT}",
    ...
    for cell in smoke:
        run_name = f"transformer_constraint_smoke_{cell.scale}_seed0"
...
def _main(argv: Sequence[str] | None = None) -> int:
    ...
    projection_cmd = subparsers.add_parser("projection-diagnostic-command")
    projection_cmd.add_argument("--task-id", type=int, required=True)
    projection_cmd.add_argument("--checkpoint-prefix", required=True)
    projection_cmd.add_argument("--output-dir", required=True)
    projection_cmd.add_argument("--run-name", required=True)
    projection_cmd.add_argument("--max-episode-steps", type=int, required=True)
    projection_cmd.add_argument("--matrix-job-id", required=True)
    runtime_cmd = subparsers.add_parser("runtime-gate-command")
    runtime_cmd.add_argument("--task-id", type=int, required=True)
    runtime_cmd.add_argument("--save-dir", required=True)
    runtime_cmd.add_argument("--run-name", required=True)
    ...
    if args.command == "diagnostic-command":
        ...
        return 0
    if args.command == "projection-diagnostic-command":
        cell = resolve_formal_cell(args.task_id)
        print(
            _command_text(
                projection_diagnostic_command(
                    cell,
                    args.checkpoint_prefix,
                    args.output_dir,
                    args.run_name,
        ...
        return 0
    if args.command == "runtime-gate-command":
        print(
            _command_text(
                runtime_gate_command(
                    resolve_runtime_gate_cell(args.task_id),
                    args.save_dir,
                    args.run_name,
                )
            )
    ...
    if args.command in {
        ...
        if args.command == "aggregate-training":
            ...
            summary_path = _write_csv(args.output_dir / "transformer_constraint_training_matrix_summary.csv", result.summary_rows)
            curve_path = _write_csv(args.output_dir / "transformer_constraint_training_curve_long.csv", result.training_curve_rows)
            print("STATUS=PASS")
        ...
        if args.command == "aggregate-eval30":
            ...
            args.output_dir.mkdir(parents=True, exist_ok=True)
            output = _write_csv(args.output_dir / "transformer_constraint_eval30_episode_rows.csv", rows)
            print("STATUS=PASS")
        ...
        args.output_dir.mkdir(parents=True, exist_ok=True)
        diagnostic_path = _write_csv(args.output_dir / "transformer_constraint_diagnostic_episode_rows.csv", result.diagnostic_rows)
        same_pass_path = _write_csv(args.output_dir / "transformer_constraint_diagnostic_same_pass_eval30_rows.csv", result.same_pass_eval_rows)
        package_path = _write_csv(args.output_dir / "transformer_constraint_diagnostic_package_summary.csv", result.package_summaries)
        print("STATUS=PASS")
