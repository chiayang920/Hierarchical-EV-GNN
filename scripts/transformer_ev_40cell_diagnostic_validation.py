#!/usr/bin/env python3
"""Prepare one transformer-EV 40-cell diagnostic reconciliation package."""
...
from scripts.transformer_ev_40cell_workflow import resolve_formal_cell
...
def prepare_validation(
    *,
    ...
    if hard_failures:
        raise ValueError(f"transformer-EV diagnostic reconciliation failed: {summary}")
    return summary
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare transformer-EV 40-cell diagnostic reconciliation evidence "
            "using the reduced-hierarchy task resolver."
        )
    )
