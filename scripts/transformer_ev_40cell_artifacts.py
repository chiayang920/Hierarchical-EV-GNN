#!/usr/bin/env python3
"""Package validation for the transformer-EV reduced-hierarchy 40-cell workflow."""
...
class TrainingPackageRecord:
    package_path: Path
    cell: workflow.TransformerEVCell
    job_id: str
    smoke: bool
...
def _expected_package_prefix(smoke: bool) -> str:
    return "m3_transformer_ev_smoke" if smoke else "m3_transformer_ev_40cell"


def _candidate_pattern(cell: workflow.TransformerEVCell, job_id: str, smoke: bool) -> str:
    return (
...
def discover_training_packages(
    ...
    job_id: object,
    *,
    cells: Sequence[workflow.TransformerEVCell] | None = None,
    smoke: bool = False,
) -> dict[int, Path]:
...
def validate_training_package(
    ...
    *,
    ...
    expected_protocol = "transformer_ev_40cell_smoke_v1" if smoke else "transformer_ev_40cell_train_v1"
...
def aggregate_training_packages(
    ...
    *,
    ...
    if training_job_id is not None:
        raise ValueError("formal transformer-EV training gate requires split training job IDs")
...
def _discover_eval_packages(
    ...
    *,
    ...
    for cell in workflow.formal_matrix():
        pattern = (
            f"m3_transformer_ev_eval30_{cell.scale}_{cell.algorithm}_seed{cell.seed}_"
...
def _discover_diagnostic_packages(
    ...
    *,
    ...
    for cell in workflow.formal_matrix():
        pattern = (
            f"m3_transformer_ev_diagnostics_{cell.scale}_{cell.algorithm}_seed{cell.seed}_"
        ...
        canonical_diagnostic_rows.append(
            {
                ...
                workflow.KEY_MECHANISM_METRIC: _parse_finite_float(row.get("global_action_fraction_at_max_active", ""), "global_action_fraction_at_max_active"),
