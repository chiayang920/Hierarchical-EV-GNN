...
for i, EV_num in enumerate(EV_list):
    # Load the data
    data = pd.read_csv(f'./Results_Analysis/raw_data/sb3_{EV_num}.csv')
    ...
    data = pd.concat([data, data_SAC, data_TD3])
...
algorithms = ['A2C', 'DDPG', 'PPO', 'TRPO', 'TQC',
              'SAC', 'TD3',
              'SAC\nGNN-FX', 'TD3\nGNN-FX', 'SAC\nEV-GNN', 'TD3\nEV-GNN']

for algo in algorithms:
    for ev_number in [25, 100, 500, 1000]:
...
# make the vertical line continue to the bottom of the plot
plt.axvline(x=6.5, color='black', linestyle='--', linewidth=1)
plt.axvline(x=8.5, color='black', linestyle='--', linewidth=1)
