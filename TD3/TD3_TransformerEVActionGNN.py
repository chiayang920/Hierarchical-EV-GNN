...
from TD3.TD3_ActionGNN_Controlled import Actor as ActionGNNActor
from TD3.TD3_ActionGNN_Controlled import Critic, resolve_device


CANONICAL_ALGORITHM_LABEL = "hierarchical_transformer_ev"
...
NON_EV_ACTION = 0.0
CHECKPOINT_METADATA_SCHEMA = "hierarchical_transformer_ev_checkpoint_v1"
...
class Actor(ActionGNNActor):
    def __init__(
        ...
        if discrete_actions != 1:
            raise ValueError(
                "TD3_TransformerEVActionGNN supports only discrete_actions=1 scalar PublicPST actions."
            )
        ...
        self.ev_allocation_score_head = torch.nn.Linear(feature_dim, 1)
    ...
    def _empty_details(self):
        return {
            ...
            "per_graph_ev_allocation_weights": [],
            "per_graph_ev_to_transformer_id": [],
        }
    ...
    def _compose_full_node_action(
        ...
        ev_allocation_scores = self.ev_allocation_score_head(
            node_embeddings[active_ev_node_indexes]
        ).reshape(-1)
        ...
        for sample_node_length in self._sample_node_lengths(state):
            ...
            if graph_ev_positions.numel() == 0 or graph_transformer_positions.numel() == 0:
            ...
            valid_ev_positions = torch.nonzero(
                ev_transformer_matched,
                as_tuple=False,
            ).reshape(-1)

            graph_ev_allocation_weights = torch.zeros(
                graph_ev_positions.numel(),
                dtype=torch.float32,
                device=self.device,
            )
            if valid_ev_positions.numel() > 0:
                valid_ev_scores = ev_allocation_scores[graph_ev_positions[valid_ev_positions]]
                valid_ev_groups = ev_transformer_positions[valid_ev_positions]
                valid_ev_allocation_weights = pyg_group_softmax(
                    valid_ev_scores,
                    index=valid_ev_groups,
                    num_nodes=graph_transformer_positions.numel(),
                )
                graph_ev_allocation_weights = graph_ev_allocation_weights.index_copy(
                    0,
                    valid_ev_positions,
                    valid_ev_allocation_weights,
                )
            ...
            hierarchy_details["per_graph_ev_allocation_weights"].append(
                graph_ev_allocation_weights
            )
            hierarchy_details["per_graph_ev_to_transformer_id"].append(
                graph_ev_to_transformer_id
            )
...
class TD3_TransformerEVActionGNN(object):
    def __init__(
        self,
        action_dim,
        max_action,
