...
EV local actions
    ↓
Final EV2Gym action vector

actor expected output -  decision signals：
1. transformer_scores
2. charger_scores
3. ev_ratios
 
 
EV2Gym only accepts -  [action_port_0, action_port_1, ..., action_port_24]


CPO → Transformer allocation 
CPOTransformerBudgetProjector -> transformer_scores => transformer_weights [torch.softmax(transformer_scores, dim=0)]
Meaning:(CPO-level allocation)
A total budget is allocated across all transformers.
Each transformer gets a proportion.
The whole transformer weights add up to 1.
[-1, 1]： -> 1.5  Clip to 1.0
...
"""
The program is designed for projecting hierarchical actor outputs into
the fixed EV2Gym action vector.

This module is a deterministic action-composition layer. It does not
learn model parameters, run TD3 updates, compute rewards, call EV2Gym,
or access the replay buffer.

Its only responsibility is to convert hierarchical actor raw outputs into:

1. active EV-node actions for the graph learning path; and
2. a fixed-size EV2Gym action vector for env.step(...).
